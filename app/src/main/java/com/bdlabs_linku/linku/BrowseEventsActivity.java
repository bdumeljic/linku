...
import com.bdlabs_linku.linku.Adapters.FragmentTabPagerAdapter;
...
public class BrowseEventsActivity extends BaseActivity implements MapEventsFragment.OnFragmentInteractionListener, EventsFragment.OnFragmentInteractionListener, ActionBar.TabListener, GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, LocationListener {
    ...
    public void onLocationChanged(Location location) {
        ...
        mLastUpdateTime = DateFormat.getTimeInstance().format(new Date());
        if (mFragmentTabPagerAdapter != null && mFragmentTabPagerAdapter.getFragmentForPosition(1) != null) {
            ((MapEventsFragment) mFragmentTabPagerAdapter.getFragmentForPosition(1)).newLocation(location);
        }
    ...
    private void refreshView() {
        ...
        mEventsAdapter.getParseAdapter().addOnQueryLoadListener(new ParseQueryAdapter.OnQueryLoadListener<Event>() {
            ...
            public void onLoaded(List<Event> events, Exception e) {
                ...
                if (mFragmentTabPagerAdapter.getFragmentForPosition(0).isAdded()) {
                    ((EventsFragment) mFragmentTabPagerAdapter.getFragmentForPosition(0)).setEmptyText();
                }

                if (mFragmentTabPagerAdapter.getFragmentForPosition(1).isAdded()) {
                    ((MapEventsFragment) mFragmentTabPagerAdapter.getFragmentForPosition(1)).setEvents(events);
                }
            }
        });
    ...
    public FragmentTabPagerAdapter getPagerAdapter() {
        return mFragmentTabPagerAdapter;
    }
